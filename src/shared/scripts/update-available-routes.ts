// src/scripts/update-routes-schedules.ts (3단계 인증)
...
async function crawlAllActiveRoutes() {
  ...
  try {
    // 1. "워밍업 1단계"
    console.log("[AUTH] 1/2: 세션 쿠키를 얻기 위해 메인 페이지 접속 중...");
    ...
    console.log("[AUTH] ✅ 1/2: 세션 쿠키 확보 완료.");

    // ▼▼▼ [신규] 워밍업 2단계: API 사용 권한 획득 ▼▼▼
    console.log(
      "[AUTH] 2/2: API 사용 권한을 얻기 위해 노선 목록 API 호출 중..."
    );
    await client.post(
      "https://www.kobus.co.kr/mrs/readRotLinInf.ajax",
      {}, // 노선 목록 API는 빈 body로 POST
      {
        headers: {
    ...
    console.log("[AUTH] ✅ 2/2: API 권한 확보 완료.\n");
    // ▲▲▲ [신규] 여기까지 ▲▲▲
